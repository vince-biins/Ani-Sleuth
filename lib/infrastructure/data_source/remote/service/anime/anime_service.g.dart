...
class _AnimeService implements AnimeService {
  ...
  Future<ApiSuccess<List<RecommendationDto>>> fetchAnimeRecommendationById({
    ...
    final _headers = <String, dynamic>{};
    const Map<String, dynamic>? _data = null;
    final _options = _setStreamType<ApiSuccess<List<RecommendationDto>>>(
      Options(method: 'GET', headers: _headers, extra: _extra)
          .compose(
            _dio.options,
            '/v4/anime/${id}/recommendations',
            queryParameters: queryParameters,
            data: _data,
          )
          .copyWith(baseUrl: _combineBaseUrls(_dio.options.baseUrl, baseUrl)),
    ...
    final _result = await _dio.fetch<Map<String, dynamic>>(_options);
    late ApiSuccess<List<RecommendationDto>> _value;
    try {
      _value = ApiSuccess<List<RecommendationDto>>.fromJson(
        _result.data!,
        (json) => json is List<dynamic>
            ? json
